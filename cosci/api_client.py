...
class APIClient:
    ...
    def request(
        ...
        self.logger.info(f"Request #{self.stats['total_requests']} starting", LogIcons.PROCESS)
        ...
        while retries < max_attempts:
            ...
            try:
                ...
                self.logger.info(f"Response: {status_code} in {attempt_time:.2f}s", icon)
                ...
                if response.status_code == 200:
                    ...
                    try:
                        ...
                        self.logger.error(f"Failed to parse JSON response: {e}", LogIcons.ERROR)
                    ...
                    self.logger.success(f"Request successful ({total_time:.2f}s total)", LogIcons.SUCCESS)
                ...
                if response.status_code == 401:
                    ...
                    self.logger.warning("Got 401 Unauthorized, refreshing token", LogIcons.AUTH)
                    ...
                    self.logger.warning(f"Rate limited (429). Waiting {wait_time}s before retry", LogIcons.TIME)
                ...
                self.logger.error(f"Request failed with status {response.status_code}", LogIcons.ERROR)
                raise APIError(
            ...
            if retries < max_attempts - 1:
                wait_time = self.RETRY_BACKOFF ** retries
                self.logger.info(f"Retrying in {wait_time}s (attempt {retries + 2}/{max_attempts})...", LogIcons.TIME)
                time.sleep(wait_time)
                retries += 1
                self.stats['total_retries'] += 1
            else:
        ...
        self.logger.error(f"Request failed after {retries + 1} attempts", LogIcons.ERROR)
    ...
    def post(self, endpoint: str, data: Dict[str, Any], **kwargs) -> Union[Dict[str, Any], List[Any]]:
        """
    ...
    def put(self, endpoint: str, data: Dict[str, Any], **kwargs) -> Union[Dict[str, Any], List[Any]]:
        """
    ...
    def log_stats(self):
        ...
        self.logger.info(f"Avg Response Time: {stats['avg_request_time']:.2f}s", LogIcons.TIME)
